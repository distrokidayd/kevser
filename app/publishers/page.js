...
export default function PublisherPanel() {
  ...
  const voteReport = async (reportId, decision) => {
    try {
      const res = await fetch("/api/vote-report", {
        method: "POST",
        headers: {
  ...
  const getApproveCount = (votes) => {
    if (!Array.isArray(votes)) return 0;
    return votes.filter((vote) => vote.decision === "approve").length;
  };

  const getRejectCount = (votes) => {
    if (!Array.isArray(votes)) return 0;
    return votes.filter((vote) => vote.decision === "reject").length;
  };
      ...
      {activeTab === "havuz" && (
        <section style={styles.card}>
          <div style={styles.sectionHeader}>
            <div>
              ...
              <p style={styles.sectionDesc}>
                ...
                2 yayıncı onayı gelirse içerik askıya alınır.
              </p>
            </div>
          ...
          <div style={styles.reportList}>
            {reports.map((report) => {
              const approveCount = getApproveCount(report.publisher_votes);
              const rejectCount = getRejectCount(report.publisher_votes);
              ...
              return (
                <div key={report.id} style={styles.reportCard}>
                  ...
                  <div style={styles.voteSummary}>
                    <span style={styles.approveCount}>
                      Onay: {approveCount}/2
                    </span>
                  ...
                  <div style={styles.progressWrap}>
                    <div
                      style={{
                        ...styles.progressApprove,
                        width: `${Math.min(approveCount, 2) * 50}%`,
                      }}
                    />
                  </div>
